...
class GpEverydayDealDetails extends BaseService
{
    ...
    public $httpOperator;
    public $requestUrl = 'http://market.finance.sina.com.cn/downxls.php?date=2016-12-16&symbol=sh601988';
    ...
    {
        parent::init();
        $this->httpOperator = new WgetTransfer('/data/gupiao/sina/');
        $this->codeTypeMap = [
    ...
    public function getDownUrl($data)
    {
        ...
        if (!isset($data[0]['code']) || !isset($data[0]['type']) || !isset($data[0]['date_str']) || strlen($data[0]['date_str']) < 8 || !isset($data[0]['id'])) {
            return false;
        }
        ...
        foreach ($data as $key => $val) {
            ...
            $dir = date('Y', $time) . DIRECTORY_SEPARATOR . date('Ymd').DIRECTORY_SEPARATOR ;
            $name = $val['code'] . '.sina.txt';
            $downDate = $val['date_str'];
            $downCode = $this->codeTypeMap[$val['type']] . $val['code'];
            $url = str_replace('{:code}', $downCode, $this->requestUrl);
            $url = str_replace('{:date}', $downDate, $url);
